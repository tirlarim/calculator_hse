...
int is_u_min(const char* s, const char* previous_s, int i) {
  if ((*s == '-' && (*previous_s == ',' || *previous_s == '(' || i == 0)) || (*s == 'u' && *previous_s == '-'))
    return 1;
  return 0;
}
...
int is_function(const WORD* word) {
  if (strcmp(word->st, "cos") == 0 || strcmp(word->st, "sin") == 0 || strcmp(word->st, "tg") == 0 || strcmp(word->st, "lg") == 0 ||
      ...
      strcmp(word->st, "phase") == 0 || strcmp(word->st, "log") == 0)
    return 1;
  return 0;
}

// int is_2arg_function(const WORD* word) {
//   if (strcmp(word->st, "pow") == 0 || strcmp(word->st, "log") == 0)
//     return 1;
//   return 0;
// }

int is_const(const WORD* word) {
  if (strcmp(word->st, "PI") == 0 || strcmp(word->st, "pi") == 0 || strcmp(word->st, "e") == 0 || strcmp(word->st, "π") == 0)
    return 1;
  return 0;
}

int compare(const char* to, const char* vs) {
  if ((*to == ',') && (*vs == '-' || *vs == '+' || *vs == '/' || *vs == '*' || *vs == '^' || *vs == '-' || *vs == '+' || *vs == 'u'))
    return 1;
  if ((*to == '-' || *to == '+') && (*vs == '/' || *vs == '*' || *vs == '^' || *vs == '-' || *vs == '+' || *vs == 'u'))
    return 1;
  if ((*to == '/' || *to == '*') && (*vs == '/' || *vs == '*' || *vs == '^' || *vs == 'u'))
    return 1;
  if (*to == '^' && (*vs == '^' || *vs == 'u'))
    return 1;
  if (*to == 'u' && *vs == 'u')
